    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Payment::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', 'paid') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('refunded_at') ? 'is-invalid' : '' }}" type="text" name="refunded_at" id="refunded_at" value="{{ old('refunded_at') }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('chargedback') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="chargedback" id="chargedback" value="1" {{ old('chargedback', 0) == 1 || old('chargedback') === null ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('affiliate_user') ? 'is-invalid' : '' }}" name="affiliate_user_id" id="affiliate_user_id" required>
                    @foreach($affiliate_users as $id => $affiliate_user)
                        <option value="{{ $id }}" {{ old('affiliate_user_id') == $id ? 'selected' : '' }}>{{ $affiliate_user }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('first_scanned') ? 'is-invalid' : '' }}" type="text" name="first_scanned" id="first_scanned" value="{{ old('first_scanned') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('last_scanned') ? 'is-invalid' : '' }}" type="text" name="last_scanned" id="last_scanned" value="{{ old('last_scanned') }}">
